...
class Tx_Solr_Facet_Facet {
		...
		if (!empty($this->configuration['requirements.'])) {
			foreach($this->configuration['requirements.'] as $name => $requirement) {
				$requirements[] = array(
					...
					'values' => \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $requirement['values']),
				);
			}
		}
	...
	protected function isRequirementMet(array $requirement) {
		...
		$requiredFacet      = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\Tx_Solr_Facet_Facet::class, $requirement['facet']);
