...
class Tx_Solr_ViewHelper_Relevance implements Tx_Solr_ViewHelper {
	...
	protected function getScore($document) {
		...
		if (is_numeric($document)) {
			...
			\TYPO3\CMS\Core\Utility\GeneralUtility::deprecationLog('You are using an old notation of the '
		...
		if (is_array($document)) {
			...
			if ($solrConfiguration['logging.']['exceptions']) {
				\TYPO3\CMS\Core\Utility\GeneralUtility::devLog('Could not resolve document score for relevance calculation', 'solr', 3, array(
