...
class Tx_Solr_PiResults_FacetingCommand implements Tx_Solr_PluginCommand {
	...
	public function __construct(Tx_Solr_PluginBase_CommandPluginBase $parentPlugin) {
		$this->search = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\Tx_Solr_Search::class);
	...
	protected function renderAvailableFacets() {
		...
		foreach ($configuredFacets as $facetName => $facetConfiguration) {
			...
			$facet = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\Tx_Solr_Facet_Facet::class,
				$facetName,
	...
	protected function renderUsedFacets() {
		...
		$queryLinkBuilder = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(\Tx_Solr_Query_LinkBuilder::class, $this->search->getQuery());
		...
		foreach ($filterParameters as $filter) {
			...
			$usedFacetRenderer = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(
